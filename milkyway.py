...
while True:
    ...
    for i in range(len(full)):
        if abs(degree - full[i]) <= 0.375:
            ...
            if (ra == 0) or (ra == 360):
                np.save(os.path.join('MilkyWay', str(0)), avgsource)
            else:
                np.save(os.path.join('MilkyWay', str(int(ra))), avgsource)
            break 
